          ...
          <table className="w-full border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-secondary border-b border-border sticky top-0 z-20">
                <th className="p-3 sm:p-5 text-left text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] border-r border-border w-24 sm:w-32 bg-secondary sticky left-0 z-30">
                  Day
                </th>
                {columns.map((col) => (
                  <th key={col.id} className="p-3 sm:p-5 min-w-[140px] sm:min-w-[180px] relative group border-r border-border last:border-r-0">
                    <div className="flex items-center gap-2 sm:gap-4">
                      <input
                        type="text"
                        value={col.name}
                        onChange={(e) => updateColumnName(col.id, e.target.value)}
                        className="bg-transparent border-none focus:ring-0 text-[10px] sm:text-xs font-bold text-foreground w-full p-0 placeholder-muted-foreground/30 uppercase tracking-widest"
                        placeholder="HABIT"
                      />
                      <button
                ...
                <tr key={i} className="border-b border-border last:border-b-0 hover:bg-secondary/40 transition-colors">
                  <td className="p-3 sm:p-5 border-r border-border bg-secondary/20 sticky left-0 z-10 backdrop-blur-sm">
                    <div className="flex flex-col items-center justify-center gap-0.5 sm:gap-1">
                      <span className="text-[8px] sm:text-[9px] font-black text-muted-foreground uppercase tracking-widest">Day {i + 1}</span>
                      <span className="text-[9px] sm:text-[10px] font-bold text-foreground/70">{getDateForDay(i)}</span>
                    </div>
                  </td>
                  {columns.map((col) => (
                    <td key={col.id} className="p-3 sm:p-5 text-center border-r border-border last:border-r-0">
                      <div 
                      ...
                      >
                        {data[`day-${i + 1}`]?.[col.id] && (
                          <svg className="w-4 h-4 sm:w-5 sm:h-5 text-primary-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={5}>
